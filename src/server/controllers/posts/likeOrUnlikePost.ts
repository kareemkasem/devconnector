...
export default async (req, res) => {
  ...
  try {
    const postToLike: any = await Post.findById(postId);
    ...
    const postAlreadyLiked: boolean = !!postToLike.likes.filter(
      (like: ObjectId) => req.user.id === like.toString()
    ).length;

    if (!postAlreadyLiked) {
      const likes: ObjectId[] = [...postToLike.likes, req.user.id];
      await Post.updateOne({ _id: postToLike.id }, { $set: { likes } });
      res.status(201).json(likes);
    } else {
      const likes: ObjectId[] = postToLike.likes.filter(
        (like: ObjectId) => like.toString() != req.user.id
      );
      await Post.updateOne({ _id: postToLike.id }, { $set: { likes } });
      res.status(201).json(likes);
    }
